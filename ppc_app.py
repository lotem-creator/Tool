...
def setup_agent():
    try:
        ...
        selected = next((m for p in priority for m in models if p in m), models[0])
        return genai.GenerativeModel(model_name=selected, system_instruction=MASTER_PROTOCOL), selected
...
def run_master_v1(category, ag_name, url):
    ...
    - HL1: "10 Best {ag_name} [Noun]" (MUST be Plural: Providers/Systems/Sites/Treatments).
    - HL2: "Top 10 {ag_name} [Noun]" (Mirror HL1 - Use exact same topic and plural noun).
    ...
    try:
        ...
        h = [smart_trim(x, 30) for x in data.get('headlines', [])]
        d = [smart_trim(x, 90, 80) for x in data.get('descriptions', [])]
    ...
    with c1:
        ...
        if st.button("🚀 Execute Protocol"):
            with st.spinner("Agent is running exhaustive V1.0 logic..."):
                ...
                if h: st.session_state.res = (h, d, mode, ag_in, camp)
